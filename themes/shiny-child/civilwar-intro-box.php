 ...
 *
 */
if($e404_options['civilwar_intro_type'] != 'none') :
	if($e404_options['civil_intro_type'] == 'twitter')
		$e404_options['intro_text'] = '<p>'.$e404_options['intro_text'].'</p>';
?>
	<div id="intro" class="<?php if($e404_options['civilwar_intro_type'] == 'twitter') echo 'twitter'; else echo 'text'; ?>-intro">
		<?php if(is_404()) : echo '<h1>'.__('Nothing Found', 'shiny').'</h1>'; else : ?>
			<?php if($e404_options['civilwar_intro_type'] == 'title') : ?>
			<?php if(is_search()) { echo '<h1>'.__('Search', 'shiny').'</h1>'; } else { the_post(); ?>
			<h1><?php the_title(); ?></h1>
			<?php rewind_posts(); } ?>
			<?php elseif($e404_options['civilwar_intro_type'] == 'title-excerpt') : ?>
			<?php if(is_search()) { echo '<h1>'.__('Search', 'shiny').'</h1>'; } else { the_post(); ?>
			<h1><?php the_title(); ?></h1>
